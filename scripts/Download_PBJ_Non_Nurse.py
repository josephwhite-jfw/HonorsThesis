...
# CMS Data Non-Nurse Staffing Levels API endpoint for the data catalog
...
# Find the PBJ Daily Non-Nurse Staffing dataset
pbj_dataset = None
for dataset in data['dataset']:
    if dataset['title'] == "Payroll Based Journal Daily Non-Nurse Staffing":
        pbj_dataset = dataset
        break

if not pbj_dataset:
    raise Exception("PBJ Daily Non-Nurse Staffing dataset not found in the CMS data catalog.")
...
for distribution in pbj_dataset.get('distribution', []):
    if distribution.get('mediaType') == "text/csv":
        ...
        filename = f"PBJ_Daily_Non_Nurse_Staffing_{temporal.replace('/', '_')}.csv"
